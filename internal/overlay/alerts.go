...
// The alert domain types live in internal/alert (their canonical home —
// the incident pipeline consumes them and must not depend on this UI
// package). These aliases keep the scanner/delivery surface source-stable.
type (
	AlertSeverity = alert.AlertSeverity
	AlertPattern  = alert.AlertPattern
	AlertSource   = alert.AlertSource
	AlertMatch    = alert.AlertMatch
)

const (
	AlertSeverityError   = alert.AlertSeverityError
	AlertSeverityWarning = alert.AlertSeverityWarning
	AlertSeverityInfo    = alert.AlertSeverityInfo

	AlertSourceProcess = alert.AlertSourceProcess
	AlertSourceBrowser = alert.AlertSourceBrowser
	AlertSourceHTTP    = alert.AlertSourceHTTP
	AlertSourceUser    = alert.AlertSourceUser
)
