...
func TestStreamEventFilterGlobalPresenceRoundTrip(t *testing.T) {
	tests := []struct {
		name      string
		...
		{name: "omitted"},
		{name: "explicit false", filter: StreamEventFilter{GlobalOverride: Bool(false)}, wantKey: true},
		{name: "explicit true", filter: StreamEventFilter{GlobalOverride: Bool(true)}, wantKey: true, wantValue: true},
		{name: "legacy true", filter: StreamEventFilter{Global: true}, wantKey: true, wantValue: true},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			data, err := json.Marshal(tc.filter)
