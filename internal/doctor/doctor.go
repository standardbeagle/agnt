...
func Run(ctx context.Context, projectPath string, d Deps) *Report {
	checks := buildChecks(projectPath, d)
	return runWithChecks(ctx, checks)
}

// buildChecks assembles the list of checks for this daemon instance.
func buildChecks(projectPath string, d Deps) []doctorCheck {
	return []doctorCheck{
		{name: "orphan_processes", fn: func(ctx context.Context) CheckResult {
			return checkOrphanProcesses(ctx, d.PIDTracker(), d.ProcessManager())
		}},
		{name: "port_conflicts", fn: func(ctx context.Context) CheckResult {
			return checkPortConflicts(ctx, projectPath, d.ProcessManager())
		}},
		{name: "daemon_health", fn: func(ctx context.Context) CheckResult {
			return checkDaemonHealthStatic(d.Version(), time.Since(d.StartedAt()), d.ClientCount())
		}},
		...
		{name: "proxy_health", fn: func(ctx context.Context) CheckResult {
			return checkProxyHealth(ctx, d.ProxyManager(), projectPath)
		}},
		{name: "missing_proxies", fn: func(ctx context.Context) CheckResult {
			return checkMissingProxies(ctx, projectPath, d.ProxyManager(), d.ScriptRegistry())
		}},
		{name: "running_with_errors", fn: func(ctx context.Context) CheckResult {
			return checkRunningWithErrors(ctx, d.ScriptRegistry(), d.AlertStore(), projectPath)
		}},
		...
		{name: "startup_errors", fn: func(ctx context.Context) CheckResult {
			return checkStartupErrors(ctx, d.StartupLog())
		}},
		{name: "process_tree", fn: func(ctx context.Context) CheckResult {
			return checkProcessTree(ctx, d.PIDTracker(), d.ProcessManager())
		}},
	}
}

// runWithChecks executes the given checks concurrently under the context deadline.
func runWithChecks(ctx context.Context, checks []doctorCheck) *Report {
	if len(checks) == 0 {
...
func checkSessionHealth(ctx context.Context, registry *session.SessionRegistry) CheckResult {
	name := "session_health"
	if registry == nil {
	...
	for _, s := range sessions {
		if s.GetStatus() == session.SessionStatusActive && now.Sub(s.LastSeen) > threshold {
...
func checkStartupErrors(ctx context.Context, store *startuplog.StartupLogStore) CheckResult {
	name := "startup_errors"
	if store == nil {
...
// checkMissingProxies detects script-linked proxies that should exist but don't.
// This catches cases where URL detection failed and no fallback-port was used.
func checkMissingProxies(ctx context.Context, projectPath string, pm *proxy.ProxyManager, registry *script.Registry) CheckResult {
	name := "missing_proxies"
	if projectPath == "" || pm == nil {
		return CheckResult{Name: name, Status: StatusOK, Message: "no project path or proxy manager"}
	}
	...
	for proxyName, proxyConfig := range cfg.Proxies {
		...
		// Check if the linked script is running
		if registry != nil {
			if entry, ok := registry.Get(proxyConfig.Script, projectPath); ok {
				if entry.State() != script.StateRunning {
					continue // Script not running, proxy not expected yet
				}
			} else {
		...
		// Check if any proxy exists for this script+project combination.
		// Proxy IDs can be either makeProcessID format or makeProxyIDFromURL format,
		// so check by prefix match.
		proxyPrefix := script.MakeProcessID(projectPath, proxyName)
		found := false
		for id := range existingIDs {
			if id == proxyPrefix || len(id) > len(proxyPrefix) && id[:len(proxyPrefix)+1] == proxyPrefix+"-" {
				found = true
				break
			}
		}
		...
		if !found {
			entry := map[string]interface{}{
				"proxy":  proxyName,
				"script": proxyConfig.Script,
			}
			if proxyConfig.FallbackPort > 0 {
				entry["fix"] = fmt.Sprintf("proxy will be created via fallback-port %d on next check", proxyConfig.FallbackPort)
			} else {
				entry["fix"] = "add fallback-port to proxy config, or fix url-matchers in .agnt.kdl"
			}
	...
	if len(missing) == 0 {
		return CheckResult{Name: name, Status: StatusOK, Message: "all expected proxies exist"}
	}

	return CheckResult{
		Name:    name,
		Status:  StatusError,
		Message: fmt.Sprintf("%d expected proxy(ies) missing -- URL detection may have failed", len(missing)),
		Details: missing,
		Fix:     "check url-matchers in .agnt.kdl match actual dev server output, or add fallback-port",
	}
}

// checkRunningWithErrors surfaces processes that are running but producing error output.
// These should NOT be restarted -- the process is working, the code needs fixes.
func checkRunningWithErrors(ctx context.Context, registry *script.Registry, alertStore *alert.ProcessAlertStore, projectPath string) CheckResult {
	name := "running_with_errors"
	if registry == nil || alertStore == nil {
		return CheckResult{Name: name, Status: StatusOK, Message: "no registry or alert store"}
	}
	...
	for _, e := range entries {
		if e.State() != script.StateRunning {
			continue
		}
		recentErrors := alertStore.Query(alert.AlertStoreFilter{
	...
	if len(errored) == 0 {
		return CheckResult{Name: name, Status: StatusOK, Message: "no processes with errors"}
	}

	return CheckResult{
		Name:    name,
		Status:  StatusWarning,
		Message: fmt.Sprintf("%d process(es) running with errors -- code needs fixes, do NOT restart", len(errored)),
		Details: errored,
	}
}
