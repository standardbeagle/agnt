...
func validateProxyInput(input ProxyInput) error {
	checks := []error{
		...
		validateStringLen("target", input.Target, maxIDLength),
		validateStringLen("direction", input.Direction, maxIDLength),
		validatePositiveLimit("width", input.Width, 100000),
		validatePositiveLimit("height", input.Height, 100000),
