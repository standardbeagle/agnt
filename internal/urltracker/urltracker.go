...
func (t *URLTracker) scanProcess(p *process.ManagedProcess) {
	...
	for _, u := range urls {
		...
		// Add new URL
		t.urls[p.ID] = append(t.urls[p.ID], u)
		t.seenURLs[p.ID][u] = true
...
// DevServerURLRegex matches localhost-like dev server URLs.
