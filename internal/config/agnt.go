...
`)
	...
	sb.WriteString("- browser diagnosis → `diagnose` (click|layout) or one audit · `__devtool.*` helpers for drill-down · skill: `/agnt:browser-debug`, `/agnt:quality-audits`\n")
	...
	sb.WriteString("\n## Debugging Workflow\n\n")
	sb.WriteString("Broken thing → highest-level tool that answers it; drill down only on failure:\n\n")
	sb.WriteString("Debugging flow: state → reproduce → ONE specialised call → drill-down → verify.\n\n")
	sb.WriteString("1. state: `currentpage {}` — what the user sees now\n")
	sb.WriteString("2. reproduce: `watch` the process/proxy or replay the interaction; `get_incidents {}` first for any error\n")
	sb.WriteString("3. ONE specialised call: `diagnose {action:\"click\"|\"layout\"}`, or one task audit: `responsive_audit`, `api_audit`, `loading_audit`\n")
	sb.WriteString("4. drill-down via the returned `next:` pointer only (`proc output`, `proxylog query`)\n")
	sb.WriteString("5. `verify_change` — confirm the fix in the browser\n")
	sb.WriteString("`release_qa` only when the user explicitly asks for release QA — never routine debugging.\n")
	sb.WriteString("`proxy exec` raw JavaScript is the last resort — raw JS only when no helper exists. Try `__devtool.*` helpers and `diagnose` first.\n")
	...
	sb.WriteString("- click dead → `diagnose {action:\"click\"}` → `next:` drill-down\n")
