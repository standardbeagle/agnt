...
`)
	...
	sb.WriteString("- browser diagnosis → dedicated audit, then `proxy exec` helpers · skill: `/agnt:browser-debug`, `/agnt:quality-audits`\n")
	...
	sb.WriteString("5. task audit: `responsive_audit`, `api_audit`, `loading_audit`, or `__devtool.auditPageQuality()`\n")
	sb.WriteString("6. failed area only: `proxy exec search`, then `describe`, then one `__devtool.*` helper\n")
	sb.WriteString("7. screenshot if visual; raw JS only when no helper exists\n")
	...
	sb.WriteString("- click dead → `interactions.getLastClickContext` → `getStacking` / `getContainer`\n")
