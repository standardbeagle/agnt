...
func TestClient_MultipleConnections(t *testing.T) {
	...
	clients := make([]*daemonclient.Client, 5)
	for i := range clients {
		clients[i] = daemonclient.NewClient(daemonclient.WithSocketPath(sockPath))
...
func TestSessionBasedCleanup(t *testing.T) {
	...
	client1 := daemonclient.NewClient(daemonclient.WithSocketPath(sockPath))
