...
| 2026-08-18 (c) | **§6a op table `addScript` row carries a REFUSED status marker** — no invariant added, retired, or renumbered; annotation only. *Was:* the operative op table's `addScript` row read as a usable op (its prose already resolved `src` to a publish-time fetch input), while the refusal of both forms was stated only in the surrounding IMPLEMENTATION STATUS / INV-14 prose below the table. *Now:* the row itself carries an inline `(status: REFUSED at publish …)` marker pointing at those notes. **Rationale:** the spec header states downstream P-tasks encode these tables **verbatim**, so an implementer reading the row alone would encode `src` as available. This is the same operative-table-repair class as the 2026-07-27 (b) row — a table annotation that corrects a reading the table gives on its own, with no change to any invariant or to the prose the reviewer ruled sufficient. | §6a |
...
| `GET /s/{token}/sub` | GET | valid token **+ daemon-minted HMAC over `(shareID, url, depth)`** | shares the artifact bucket inbound, the per-origin bucket outbound | R: one allowlisted subresource (CSS/image/font) of the proxied upstream, fetched under INV-13; no write. Added 2026-08-19, INV-16..19 |
...
| **INV-16** (2026-08-19) | The subresource route (`/s/{token}/sub`) serves only references the daemon itself derived from a guarded fetch of that share's **publisher-named** upstream (its document, or a stylesheet fetched from it). Each reference carries an HMAC bound to `(shareID, absolute-url, depth)`; a viewer-composed URL — forged/absent MAC, another share's reference, an altered target, a depth downgrade — is refused **before any socket opens**. Every subresource fetch runs the full INV-13 guard on the origin **and every redirect hop**, fail-closed, sharing one walk implementation with the document fetch so a hop check cannot exist on one route and not the other. | Forged/absent MAC → 404 with **zero dial** (asserted via the test seam's dial log). A subresource redirecting to `169.254.169.254` is refused at that hop with the guard's own verdict, having dialed only the first hop. Revoke → every subresource URL 404s atomically with the artifact (INV-4). Mutation: deleting the MAC check turns those 404s into 200s/502s. |
| **INV-17** (2026-08-19) | Subresource responses are media-type **allowlisted**: `text/css`, `image/*` except `image/svg+xml`, and `font/*` plus legacy `application/font-woff`. HTML, JS, JSON, SVG, and anything unrecognised refuse loudly (502) **before the body is read**, and the served `Content-Type` is composed from the allowlisted type rather than echoed from the upstream. `script-src` remains hash-sources-only on **every** public response, so no subresource can be served into a script-executing context. | Upstream answers a signed reference with `text/html` / `application/javascript` / `image/svg+xml` → 502 and the body never appears in the response. Header sweep across every response shape: no `script-src` contains `'self'`, a host source, or any `unsafe-*`. |
| **INV-18** (2026-08-19) | `style-src 'unsafe-inline'` appears on **exactly one** response shape — the proxied live-upstream artifact document — and nowhere else (the self-contained shell keeps its nonce; JSON, feedback, refusal, and subresource responses keep bare `'self'`). The fetch-governing directives (`img-src`, `connect-src`, `font-src`/`default-src`) do **not** widen anywhere. | Header table across every shape the plane can produce, with the expected directive written as a **literal** rather than as the constant under test. Mutation: narrowing the constant fails only the proxied-artifact rows. |
| **INV-19** (2026-08-19) | Subresource work is bounded and every refusal is loud: 2 MiB per subresource (refused, never truncated), 64 distinct references rewritten per document or stylesheet (the excess left exactly as the upstream wrote it, with the refusal logged), CSS nesting depth 2, and both existing rate buckets covering the route (inbound per `(share, IP)`, outbound per upstream origin). No cap is enforced by silent truncation. | Over-cap body → 502, not a partial body. Reference #65 is served byte-identical to the upstream's own markup. A stylesheet reached at the depth cap serves but mints no deeper reference. |
...
| `addScript` **(status: REFUSED at publish — see the IMPLEMENTATION STATUS / INV-14 notes below)** | `{op:"addScript", src?, code?}` | append an **inline** `<script>` (never `<script src>`) to the variant root | ≤16384 B per authored revision, counted on the **final inlined body**; the body executes **only** if its `sha256` is in the revision's pinned `script-src` hash set (INV-12). `src` is a **publish-time fetch input, not a runtime attribute**: it must be `https:`, pass §4a (INV-13), and its fetched body is inlined at publish time so its hash joins the pinned set — see below. **`src` is NOT implemented and is refused at publish** (2026-07-29 amendment-log row); on a published walkthrough **both `src` and `code` are refused** (INV-14, 2026-08-18). This row documents the recorded plan, not shipped behaviour — do not encode `src` as usable |
...
| **Guarded subresource proxying + proxied-artifact `style-src`** — signed, share-bound references for CSS/images/fonts served from `'self'` through the same INV-13 guard; JS excluded permanently; `style-src 'self' 'unsafe-inline'` on the proxied document alone (`docs/superpowers/specs/2026-08-19-subresource-proxying-style-src-design.md`) | INV-16, INV-17, INV-18, INV-19 | task `01KYQFAZSV6AQTKASQHYVPKJZC` (`internal/proxy/public_subresource.go`, `public_routes.go`, `internal/publish/subresource.go`) |
