...
| **INV-9** | The share token is **redacted** from every log, event, incident, and outbound `Referer`/`Referrer-Policy` surface. Only its hash prefix (≤8 hex) may appear for correlation. | grep test over log/event emitters; `Referrer-Policy: no-referrer` on artifact responses. |
...
| **INV-11** | Injected-bundle **CSP is authoritative** over the third-party upstream: the proxy **replaces** the upstream CSP (as auth-breakout already rewrites upstream frame/CSP headers, `docs/auth-breakout.md`) with a policy that forbids `unsafe-inline`/`unsafe-eval` and pins the bundle to `self` + content hash. | Response header assertion; injected inline handlers are refused by the browser. |
...
| Property | Rule |
|---|---|
| Source | `crypto/rand.Read` (CSPRNG). Matches repo crypto convention (`crypto/rand` in `internal/sshclient`; `crypto/sha256` throughout `internal/incident`, `internal/store/url.go`). **Never** `math/rand`. |
...
| Hash at rest | Persist `sha256(token)` hex only (`daemon-architecture.md` never persists secrets; mirror `blob_store.go`/`store/url.go` sha256 use). Plaintext never touches disk, logs, or events. |
...
| Redaction | Token never appears in `debug.Log`, session log, incident events, proxy traffic log, or `Referer`. Correlation uses `hash[:8]` only (INV-9). Artifact responses set `Referrer-Policy: no-referrer` so the token path never leaks to the upstream's third-party subresource requests. |
...
(auth-breakout already rewrites upstream frame/CSP headers, `docs/auth-breakout.md`).

| Header | Value (public plane) | Why |
|---|---|---|
| `Content-Security-Policy` | `default-src 'self'; script-src 'self' 'sha256-<bundle-hash>'; style-src 'self' 'nonce-<n>'; img-src 'self' data: https:; connect-src 'self'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'; object-src 'none'` | No `unsafe-inline`, no `unsafe-eval` (INV-6/INV-11). Bundle pinned to `self` + content hash (matches `injector.go` content-addressed asset path). `connect-src 'self'` blocks feedback exfil to third parties. |
...
| **Public routes** + endpoint matrix + deny-by-default + header policy + scope isolation (§2, §4) | INV-1, INV-2, INV-10, INV-11 | **P7** — public routes |
...
| **End-to-end**: token guess, revoke-kills-all, forbidden-module build fail, XSS/JS-injection attempts, restart survival | INV-1…INV-11 (integration) | **P10** — e2e |
