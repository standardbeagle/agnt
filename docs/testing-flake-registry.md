...
| All `NewForTest(t, DaemonConfig{})` callers (13, incl. `TestIncidentDigest_EndToEnd_StormToStream`, `TestProcessAlertCallbackUsesImmutableOwnerAndDropsUnresolved`, the `TestChaosInjectedFault_*` / `TestSwallowHeuristic_*` / `TestIngestProcessAlert_*` cluster) | `internal/daemon` (`test_helpers.go`) | **Shared control-socket path, NOT a timing flake.** An empty `SocketPath` in `DaemonConfig{}` falls through `hub.New` to `protocol.DefaultSocketPath()` — the PRODUCTION socket — so every such test bound the SAME fixed path. In the real `-p 1` gate (one process) each test's `t.Cleanup` Stop frees the socket before the next binds it, so the collision stays latent and sequential. Two concurrent `daemon.test` binaries (the deliberate repro in the owning task) race for that one path; the loser's `bootstrap` fails **instantly** with `failed to start hub: daemon already running` — an instant failure at 0.00s, not a timeout. Whichever `DaemonConfig{}` test happens to be mid-`NewForTest` when the other process holds the socket loses, so the failing test set is nondeterministic run-to-run (the owning task named two; a fresh repro on 2026-08-18 hit five different ones — same mechanism). It also means these tests would fight a real `agnt` daemon running on the dev box. | 2026-08-18 (`NewForTest` now defaults an empty `SocketPath` to `filepath.Join(t.TempDir(), "d.sock")`, mirroring the existing hermetic `PublishDir`/`FeedbackDir` defaults in the same helper). Removes the shared global path, closing the class even under two concurrent binaries — verified green (0 `daemon already running`) across two concurrent full-package runs where the pre-fix binary failed. No assertion or timing touched, so teeth are unchanged |
| `TestProxyWaitFor_PortProbeSurvivesAutostart` | `internal/daemon` (`proxy_waitfor_persist_test.go`) | **Harness TOCTOU, root-caused; dominant vector fixed, residual intrinsic — see the non-determinizable note below.** Failed ONCE on an unloaded HEAD run. The test needs a specific port DOWN for an interval (so the readiness gate stays closed through `require.False`) then UP (bind it, probe must detect it). It picks that port with `ephemeralPort` (bind `:0`, close) and binds the same port itself ~40 lines later — the misuse `ephemeralPort`'s own doc warns against. During the deliberately-unbound interval any `:0` bind can steal the freed port: if stolen-and-bound the TCP-connect probe (`ready_signaler.go` `StartPortProbe`) sees it up early and `require.False` fails; if merely held, the later `net.Listen` fails. A fixed port that must be down-then-up is intrinsically racy on a shared host — a connect probe cannot see a held reservation as "down." | 2026-08-18 (reorder: bind this test's own two ephemeral sockets — the `httptest` backend and the proxy listen port — BEFORE choosing the probe port, so `ephemeralPort` is guaranteed distinct from them). This removes the test's own binds from the theft window; the residual parallel-sibling `:0` window is intrinsic and registered below. Teeth verified by pointing `StartPortProbe` at an already-cancelled context (simulating the autostart-ctx-death bug it guards): the reordered test still fails `Condition never satisfied` |
...
### Residual TOCTOU window in `TestProxyWaitFor_PortProbeSurvivesAutostart`

- `TestProxyWaitFor_PortProbeSurvivesAutostart` (`internal/daemon`)

Root-caused and mostly fixed on 2026-08-18 (see the determinized table above),
but one window is **intrinsic and cannot be closed** without changing the
production probe's contract. The test deliberately needs a specific port to be
DOWN for an interval (so the readiness gate stays closed through the
`require.False` assertion) and then UP (so the surviving probe can detect the
late bind). On a shared host a TCP-connect probe reads "down" as "nothing is
listening" — so the port cannot simultaneously be reserved-against-theft (a
held listener answers the connect and would read as "up," breaking the gate's
initial closed state). The 2026-08-18 reorder removed this test's own two
ephemeral binds (`httptest` backend, proxy listen port) from the theft window;
what remains is that a **parallel sibling test's** `:0` allocation can, with low
probability, land on the freed probe port during the unbound interval. That is
the one-off unloaded failure the owning task observed. This is a harness
property of "a fixed port that must be down-then-up," not a product defect and
not a wall-clock timing issue — deliberately **not** mitigated by widening the
4s `require.Eventually` ceiling (a widen would mask nothing here anyway, since
the failure is a stolen port, not slow detection). If it recurs, the only
further hardening is to move the probe port out of the OS ephemeral range,
which trades the sibling window for a rare cross-binary fixed-port collision —
judged not worth it while occurrences stay one-off.
