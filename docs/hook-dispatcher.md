...
**Cost contract**: unloaded p99 cold exit target ≤5ms, measured ~270µs in `cmd/agnt/hook_bench_test.go` (`BenchmarkHookColdExit`, opt-in, reference baseline for the daemon contribution); CI enforces `hook_p99 ≤ 4·baseline_p99 + 50ms` against the same-run baseline to track the machine floor under load and catch gross regressions. Dispatcher uses 50ms hard deadline on daemon round-trip, opens dedicated short-lived client (no shared state), writes entire enqueue path daemon-side as single mutex push into 1024-slot ring buffer.

**CI guard** (`TestHook_LatencyWithinBaselineFactor`, `cmd/agnt/hook_latency_test.go`): the contract is enforced in the default test run, not left to the opt-in benchmark. Absolute wall-clock bounds on this path flake under load — that is why the older `TestHook_LatencyAgainstWarmDaemon` max-of-50 budget was widened 50ms→100ms→1s and demoted to an order-of-magnitude smoke test; the scheduler can stall any single sample for hundreds of ms on a saturated box, so max/p99 wall-clock is not a load-stable statistic. The guard instead calibrates against a same-run baseline (300 connect+write+read round trips to a trivial in-process echo socket, dual-dial to mirror the dispatcher's `Connect` + `HookSend` shape). The baseline absorbs exactly the load-sensitive component (socket setup + scheduler jitter), so under CPU load both the baseline and the dispatcher measurement inflate together and the derived bounds inflate with them. It asserts two self-calibrated bounds over 300 dispatches: a **median** bound (`hook_p50 ≤ 6·baseline_p50 + 5ms`) that catches a distribution-wide regression (extra round trip, added serialization, per-call allocation — the median is a lower bound on p99, so a median approaching 5ms means the contract is already blown), and a **tail** bound (`hook_p99 ≤ 4·baseline_p99 + 50ms`) whose 50ms floor catches a fixed per-call cost that lifts the dispatcher tail without lifting the baseline tail. Verified 25/25 green under 2× CPU oversubscription (hook p50 stayed ≤253µs).
