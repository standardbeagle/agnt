...
**JS/DOM tier** (`make test-js`): the shipped instrumentation scripts in `internal/proxy/scripts` also have a vitest + jsdom suite at `internal/proxy/scripts/jstest`, which evaluates the real script files against a real document (`load-audit.js` reads the shipped bytes — nothing is re-implemented). It is deliberately outside `make test` so the Go suite never depends on a node install, and the target loud-skips when npm is absent. jsdom answers for stylesheets, attributes and computed styles; anything needing layout, paint or renderer timing belongs to the `chromee2e` tier instead. Every Go source guard whose behaviour lives in that tier says so in its doc comment, and `TestModernCSS_EveryFeatureHasDOMCoverage` fails when a detection ships with no case there. Conventions: `.claude/rules/testing-conventions.md` § 1.
